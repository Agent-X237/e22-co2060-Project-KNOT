...
/**
 * 11. 2-Step Verification Prompt Notification
 */
async function sendBookingVerificationPromptNotification(pool, { booking, recipientEmail, recipientName, deadlineStr }) {
  const fromAddress = process.env.EMAIL_FROM || process.env.SMTP_USER || 'minhaj.dssc2@gmail.com';
  const email = recipientEmail || 'e22237@eng.pdn.ac.lk';
  const name = recipientName || 'Requester';

  const subject = `[Action Required] 2-Step Verification: Confirm Booking for ${booking.title} (#BKG-${String(booking.id).padStart(4, '0')})`;
  const bodyHtml = wrapHtmlTemplate(
    subject,
    'Please confirm your lecture hall booking on the KNOT App before deadline.',
    `
      <span class="badge badge-amber">⚠️ 2-Step Verification Required</span>
      <h3 style="margin: 0 0 12px 0; color: #0f172a; font-size: 18px;">Hello ${name},</h3>
      <p style="font-size: 14px; line-height: 1.6; color: #334155; margin-bottom: 20px;">
        Your booking for <strong>${booking.title}</strong> requires a 2-step confirmation to lock in your reserved slot. Please log in to the KNOT app and confirm or cancel your booking.
      </p>

      <div class="info-box" style="border: 2px solid #f59e0b; background: #fffbeb;">
        <div class="info-row"><span class="info-label">Venue / Room:</span> <span class="info-value">${booking.title}</span></div>
        <div class="info-row"><span class="info-label">Reserved Slot:</span> <span class="info-value">${booking.time_display}</span></div>
        <div class="info-row"><span class="info-label">Confirmation Deadline:</span> <span class="info-value" style="color: #dc2626; font-weight: 800;">${deadlineStr}</span></div>
      </div>

      <div class="notes-box">
        <strong>Important:</strong> If you do not confirm this booking on the app before the deadline, the system will automatically cancel the reservation and free the slot for other users.
      </div>
    `
  );
  ...
  try {
    await transporter.sendMail({ from: fromAddress, to: email, subject, html: bodyHtml });
    console.log(`[EmailService] Sent 2-STEP_VERIFICATION_PROMPT email for Booking #${booking.id} to (${email})`);
  } catch (err) {
    console.error('[EmailService] Error sending verification prompt email:', err.message);
  }
  ...
  await logNotification(pool, {
    ...
    recipientName: name,
    subject,
    eventType: 'VERIFICATION_PROMPT',
    messageBody: `2-Step verification required for ${booking.title}. Deadline: ${deadlineStr}.`
  });
}

/**
 * 12. 2-Step Verification Expired & Slot Freed Notification
 */
async function sendBookingVerificationExpiredNotification(pool, { booking, recipientEmail, recipientName }) {
  const fromAddress = process.env.EMAIL_FROM || process.env.SMTP_USER || 'minhaj.dssc2@gmail.com';
  const email = recipientEmail || 'e22237@eng.pdn.ac.lk';
  const name = recipientName || 'Requester';

  const subject = `[Slot Freed] Booking Auto-Cancelled: ${booking.title} (#BKG-${String(booking.id).padStart(4, '0')})`;
  const bodyHtml = wrapHtmlTemplate(
    subject,
    'Your booking was automatically cancelled due to missed 2-step verification.',
    `
      <span class="badge badge-purple">❌ 2-Step Verification Expired</span>
      <h3 style="margin: 0 0 12px 0; color: #0f172a; font-size: 18px;">Hello ${name},</h3>
      <p style="font-size: 14px; line-height: 1.6; color: #334155; margin-bottom: 20px;">
        Your booking for <strong>${booking.title}</strong> (${booking.time_display}) has been automatically cancelled because 2-step verification was not completed before the deadline.
      </p>

      <div class="info-box">
        <div class="info-row"><span class="info-label">Venue / Room:</span> <span class="info-value">${booking.title}</span></div>
        <div class="info-row"><span class="info-label">Slot:</span> <span class="info-value">${booking.time_display}</span></div>
        <div class="info-row"><span class="info-label">Status:</span> <span class="info-value" style="color: #dc2626;">Cancelled & Slot Freed</span></div>
      </div>

      <p style="font-size: 13px; color: #64748b;">The reserved slot has been released back into the available booking directory.</p>
    `
  );

  try {
    await transporter.sendMail({ from: fromAddress, to: email, subject, html: bodyHtml });
    console.log(`[EmailService] Sent 2-STEP_VERIFICATION_EXPIRED email for Booking #${booking.id} to (${email})`);
  } catch (err) {
    console.error('[EmailService] Error sending verification expired email:', err.message);
  }
  ...
  await logNotification(pool, {
    ...
    messageBody: `Booking for ${booking.title} auto-cancelled due to missed 2-step verification.`
  });
}

/**
 * 13. 2-Step Verification Confirmed Notification
 */
async function sendBookingVerificationConfirmedNotification(pool, { booking, recipientEmail, recipientName }) {
  const fromAddress = process.env.EMAIL_FROM || process.env.SMTP_USER || 'minhaj.dssc2@gmail.com';
  const email = recipientEmail || 'e22237@eng.pdn.ac.lk';
  const name = recipientName || 'Requester';

  const subject = `[Confirmed] 2-Step Verification Verified: ${booking.title} (#BKG-${String(booking.id).padStart(4, '0')})`;
  const bodyHtml = wrapHtmlTemplate(
    subject,
    'Your lecture hall booking is 100% verified & locked.',
    `
      <span class="badge badge-emerald">✓ 2-Step Verification Complete</span>
      <h3 style="margin: 0 0 12px 0; color: #0f172a; font-size: 18px;">Hello ${name},</h3>
      <p style="font-size: 14px; line-height: 1.6; color: #334155; margin-bottom: 20px;">
        Thank you for completing the 2-step verification! Your booking for <strong>${booking.title}</strong> is officially verified and locked in.
      </p>

      <div class="info-box" style="border: 2px solid #10b981; background: #ecfdf5;">
        <div class="info-row"><span class="info-label">Venue / Room:</span> <span class="info-value" style="color: #047857;">${booking.title}</span></div>
        <div class="info-row"><span class="info-label">Reserved Slot:</span> <span class="info-value">${booking.time_display}</span></div>
        <div class="info-row"><span class="info-label">2-Step Status:</span> <span class="info-value" style="color: #047857;">VERIFIED & LOCKED</span></div>
      </div>
    `
  );
  ...
  try {
    await transporter.sendMail({ from: fromAddress, to: email, subject, html: bodyHtml });
    console.log(`[EmailService] Sent 2-STEP_VERIFICATION_CONFIRMED email for Booking #${booking.id} to (${email})`);
  } catch (err) {
    console.error('[EmailService] Error sending verification confirmed email:', err.message);
  }
  ...
  await logNotification(pool, {
    ...
    messageBody: `Booking for ${booking.title} 2-step verified and locked.`
  });
}
...
module.exports = {
  ...
  sendBookingVerificationPromptNotification,
  sendBookingVerificationExpiredNotification,
  sendBookingVerificationConfirmedNotification
};
